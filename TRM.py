...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        # 首先映射分头，然后计算attn_scores，然后计算attn_value;
...
class FeedForward(nn.Module):
    ...
    def forward(self, inputs, pre_input):
        # inputs : [batch_size, len_q, d_model]
        return self.layer_norm(inputs + pre_input)
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs):
        # 这里我们的 enc_inputs 形状是： [batch_size, source_len]
...
if __name__ == '__main__':
    ...
    # 句子的输入部分，
    sentences = ['今天 天气 真 好 P', 'S what a nice day', 'what a nice day E']

    # Transformer Parameters
    # Padding Should be Zero
    # 构建词表
    src_vocab = {'P': 0, '今天': 1, '天气': 2, '真': 3, '好': 4}
